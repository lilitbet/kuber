    ...
    <div class="container border rounded shadow p-3 mb-5 bg-body-tertiary" style="min-height: 80vh;">
			...
			<form action="note_update.php" method="post">
		  	<div class="input-group mb-3">
		  		<?php  
    				$decode_id = base64_decode($_GET['pid']);

    				require("connectDB.php");

    				$result = mysqli_query($link, "SELECT * FROM pages WHERE id = {$decode_id}");
    				$row = mysqli_fetch_row($result);
    				echo "<input type='text' class='form-control' value='{$row[2]}' name='title'>
    				<input type='text' class='form-control' value='{$row[0]}' name='pid' hidden>
    		</div>		  
		  	<div class='mb-3'>
		    	<textarea class='form-control' style='height: 60vh' name='text'>{$row[4]}</textarea>
		  	</div>
		  	<button type='submit' class='btn btn-success'>Сохранить и выйти</button>
				<a href='note_delete.php?pid={$decode_id}' class='btn btn-danger'>Удалить</a>";
    			?>
